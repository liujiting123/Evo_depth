...
Training follows the same **LeRobot v2.1-style** layout as [Evo-1](https://github.com/MINT-SJTU/Evo-1): each dataset root should contain `meta/` (e.g. `tasks.jsonl`, `episodes.jsonl`, stats) and `data/` / `videos/` as consumed by `lerobot_dataset_pretrain_mp.py`. This repo uses a **three-stage** schedule (see `[train.sh](Evo_depth/train.sh)`): (1) action head only, (2) DA3 + action head, (3) VLM + DA3 + action head full fine-tuning.
