...
from lerobot.policies.evo1.depth_anything_v3 import DAV3Module


class DPTLinearFilM(nn.Module):
    """Project DA3 depth features into per-sample FiLM (gamma, beta).

    Mirrors the original Evo_depth helper: pool the DA3 token sequence to
    a single [1, 1, embed_dim] vector, then produce two [1, 1, embed_dim]
    modulation tensors that broadcast across the InternVL3 token sequence.
    """

    def __init__(self, dim_in: int = 1536, dim_out: int = 896):
        super().__init__()
        self.proj = nn.Linear(dim_in, dim_out)
        self.relu = nn.ReLU(inplace=True)
        self.norm = nn.LayerNorm(dim_out)
        self.proj_c = nn.Sequential(nn.Linear(dim_out, dim_out * 2))
...
class InternVL3Embedder(nn.Module):
    def __init__(
        ...
        use_da3: bool = False,
        da3_checkpoint_dir: str | None = None,
    ):
        super().__init__()
        ...
        if self.use_da3:
            self.dpt_embedder = DAV3Module(checkpoint_dir=da3_checkpoint_dir)
            self.dpt_embedder.to(self._requested_device)
            self.dpt_linear_film = DPTLinearFilM().to(self._requested_device)
    ...
    def _preprocess_single_image(self, image: Image.Image | torch.Tensor) -> torch.Tensor:
        ...
        tile_tensors = torch.stack([TF.to_tensor(tile) for tile in tiles])
        mean = torch.tensor(IMAGENET_MEAN).view(1, 3, 1, 1)
        std = torch.tensor(IMAGENET_STD).view(1, 3, 1, 1)
        normalized = (tile_tensors - mean) / std
        return normalized.to(device=self.device, dtype=torch.bfloat16)
    ...
    def get_fused_image_text_embedding_from_tensor_images(
        ...
        if self.use_da3:
            masks_on_device = image_masks.to(device=self.device)
            batch_size = len(image_tensors_batch)
            da3_needs_grad = any(p.requires_grad for p in self.dpt_embedder.parameters())
            modulated_slices = []
            for i in range(batch_size):
                sample_images = image_tensors_batch[i]
                sample_mask = masks_on_device[i]
                da3_inputs = []
                for img in sample_images:
                    if isinstance(img, torch.Tensor):
                        ...
                        arr = torch.from_numpy(np.array(pil_rgb)).float() / 255.0
                        t = arr.permute(2, 0, 1)
                    da3_inputs.append(t)

                if da3_needs_grad and torch.is_grad_enabled():
                    def _da3_forward(mask_in=sample_mask, inputs=da3_inputs):
                        return self.dpt_embedder(inputs, mask_in)
                    dummy = torch.zeros(1, device=self.device, requires_grad=True)
                    dpt_feats = torch_checkpoint(lambda _d: _da3_forward(), dummy, use_reentrant=False)
                else:
                    with torch.no_grad():
                        dpt_feats = self.dpt_embedder(da3_inputs, sample_mask)
                gamma, beta = self.dpt_linear_film(dpt_feats.to(torch.float32))
                gamma_f = gamma.to(fused_hidden.dtype)
                beta_f = beta.to(fused_hidden.dtype)
                modulated_slices.append(fused_hidden[i : i + 1] * gamma_f + beta_f)
            fused_hidden = torch.cat(modulated_slices, dim=0)
