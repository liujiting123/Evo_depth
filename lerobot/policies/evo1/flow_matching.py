...
class CategorySpecificLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor, category_id: torch.LongTensor):
        if self.num_categories <= 1:
            return self.linear(x.to(dtype=self.linear.weight.dtype))

        orig_shape = x.shape
        x_flat = x.reshape(-1, orig_shape[-1])
        if category_id.dim() == 0:
            cid = category_id.item()
            weight = self.weight[cid]
            out = x_flat.to(dtype=weight.dtype) @ weight + self.bias[cid]
        else:
            category_id = category_id.view(-1)
            weight_selected = self.weight[category_id]
            bias_selected = self.bias[category_id]
            x_flat = x_flat.to(dtype=weight_selected.dtype)
...
class MultiEmbodimentActionEncoder(nn.Module):
    ...
    def forward(self, action_seq: torch.Tensor, category_id: torch.LongTensor):
        ...
        if category_id.dim() == 0:
            cat_ids = category_id.repeat(horizon * batch_size)
        else:
            cat_ids = category_id.unsqueeze(1).repeat(1, horizon).reshape(batch_size * horizon)
        out = self.activation(self.W1(x, cat_ids))

        pos_enc = self.pos_encoding(horizon).to(out.device)
...
class FlowmatchingActionHead(nn.Module):
    ...
    def _expand_action_mask(
        ...
        if action_mask.dim() == 2:
            ...
            if action_mask.shape == (batch_size, expected_last_dim):
                mask_seq = action_mask.view(batch_size, self.horizon, per_action_dim)
            elif action_mask.shape == (batch_size, per_action_dim):
                mask_seq = action_mask.unsqueeze(1).expand(batch_size, self.horizon, per_action_dim)
            else:
                raise ValueError(
                    f"action_mask shape {tuple(action_mask.shape)} is incompatible with "
                    f"(batch={batch_size}, horizon={self.horizon}, per_action_dim={per_action_dim})"
                )
        elif action_mask.dim() == 3:
            if action_mask.shape == (batch_size, self.horizon, per_action_dim):
                mask_seq = action_mask
            elif action_mask.shape == (batch_size, 1, per_action_dim):
                mask_seq = action_mask.expand(batch_size, self.horizon, per_action_dim)
            else:
                raise ValueError(
                    f"action_mask shape {tuple(action_mask.shape)} is incompatible with "
                    f"(batch={batch_size}, horizon={self.horizon}, per_action_dim={per_action_dim})"
            ...
            raise ValueError(f"action_mask must be 2D or 3D, got dim={action_mask.dim()}")

        return mask_seq.to(device=device, dtype=dtype)
    ...
    def get_action(
        ...
        action_seq = action.view(batch_size, self.horizon, per_action_dim) if self.horizon > 1 else action.view(
            batch_size, 1, per_action_dim
        )
