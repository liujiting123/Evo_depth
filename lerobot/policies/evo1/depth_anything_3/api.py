...
from lerobot.policies.evo1.depth_anything_3.cfg import create_object, load_config
from lerobot.policies.evo1.depth_anything_3.registry import MODEL_REGISTRY
from lerobot.policies.evo1.depth_anything_3.specs import Prediction
from lerobot.policies.evo1.depth_anything_3.utils.geometry import affine_inverse
from lerobot.policies.evo1.depth_anything_3.utils.io.input_processor import InputProcessor
from lerobot.policies.evo1.depth_anything_3.utils.io.output_processor import OutputProcessor
from lerobot.policies.evo1.depth_anything_3.utils.logger import logger
from lerobot.policies.evo1.depth_anything_3.utils.pose_align import align_poses_umeyama
...
class DepthAnything3(nn.Module, PyTorchModelHubMixin):
    ...
    def _export_results(
        ...
        start_time = time.time()
        from lerobot.policies.evo1.depth_anything_3.utils.export import export  # lazy
    ...
    def _get_dpt_embeddings(
        ...
        # x_list = [c.cpu().numpy() for img in x for c in img]
        
        x_list = []
        for i in range(len(x)):
            img_tensor = x[i].cpu()
            img_uint8 = (img_tensor.permute(1, 2, 0).numpy() * 255).astype(np.uint8)  # (H,W,3)
        ...
        imgs_cpu, extrinsics, intrinsics = self._preprocess_inputs(
            x_list, extrinsics, intrinsics, process_res, process_res_method
        )
        
        # Prepare tensors for model
        imgs, _,_ = self._prepare_model_inputs(imgs_cpu, extrinsics, intrinsics)
        
        
        return self.model._get_dpt_embedding(
            imgs,
        )
