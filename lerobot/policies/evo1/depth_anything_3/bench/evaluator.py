...
from lerobot.policies.evo1.depth_anything_3.bench.print_metrics import MetricsPrinter
from lerobot.policies.evo1.depth_anything_3.utils.parallel_utils import parallel_execution
from lerobot.policies.evo1.depth_anything_3.bench.registries import MV_REGISTRY
from lerobot.policies.evo1.depth_anything_3.utils.constants import EVAL_REF_VIEW_STRATEGY
...
class Evaluator:
    ...
    def _compute_pose_with_gt(self, result_path: str, gt_meta: Dict) -> TDict[str, float]:
        ...
        """
        from lerobot.policies.evo1.depth_anything_3.bench.dataset import _wait_for_file_ready
        from lerobot.policies.evo1.depth_anything_3.bench.utils import compute_pose
        from lerobot.policies.evo1.depth_anything_3.utils.geometry import as_homogeneous
...
if __name__ == "__main__":
    ...
    from lerobot.policies.evo1.depth_anything_3.cfg import load_config
...
Usage:
  python -m model.depth_anything_3.bench.evaluator [OPTIONS] [KEY=VALUE ...]
...
Examples:
  # Use default config
  python -m model.depth_anything_3.bench.evaluator

  # Override model path
  python -m model.depth_anything_3.bench.evaluator model.path=depth-anything/DA3-LARGE
  ...
  CUDA_VISIBLE_DEVICES=0,1,2,3 python -m model.depth_anything_3.bench.evaluator
  ...
  python -m model.depth_anything_3.bench.evaluator eval.eval_only=true

  # Only print saved metrics
  python -m model.depth_anything_3.bench.evaluator eval.print_only=true
    ...
    if print_only:
        ...
        if len(gpu_list) > 1 and not is_worker:
            ...
            # Build base command
            base_cmd = [sys.executable, "-m", "model.depth_anything_3.bench.evaluator"]
