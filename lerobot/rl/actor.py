...
def _step_action_tensor(action: torch.Tensor) -> torch.Tensor:
    action = action.detach().clone().to(dtype=torch.float32)
    if action.ndim > 1 and action.shape[0] == 1:
        action = action.squeeze(0)
    return action


def _stack_action_chunk(actions: list[torch.Tensor], chunk_size: int) -> torch.Tensor:
    if not actions:
        raise RuntimeError("Cannot build an empty action chunk")
    padded = [action.detach().clone() for action in actions]
    while len(padded) < chunk_size:
        padded.append(padded[-1].clone())
    return torch.stack(padded[:chunk_size], dim=0).unsqueeze(0)


def _build_rlt_online_chunk_transitions(
    *,
    policy: RLTTD3Policy,
    ...
    for start in range(len(episode_steps)):
        chunk_steps = episode_steps[start : start + rl_chunk_size]
        if not chunk_steps:
            continue

        chunk_actions = _stack_action_chunk(
            [_step_action_tensor(step["executed_action"]) for step in chunk_steps],
            rl_chunk_size,
        )
        chunk_reward = 0.0
        chunk_done = False
        chunk_truncated = False
        chunk_intervention = False
        chunk_penalty = 0.0
        terminal_offset = None
        for offset, step in enumerate(chunk_steps):
            chunk_reward += (discount**offset) * float(step["reward"])
            chunk_done = chunk_done or bool(step["done"])
            chunk_truncated = chunk_truncated or bool(step["truncated"])
            chunk_intervention = chunk_intervention or bool(step["is_intervention"])
            chunk_penalty += float(step["discrete_penalty"])
            if (step["done"] or step["truncated"]) and terminal_offset is None:
                terminal_offset = offset

        if terminal_offset is not None:
            bootstrap_start = start + terminal_offset + 1
            next_state = _clone_state(chunk_steps[terminal_offset]["next_state"])
        elif start + rl_chunk_size < len(episode_steps):
            bootstrap_start = start + rl_chunk_size
            next_state = _clone_state(episode_steps[start + rl_chunk_size - 1]["next_state"])
        else:
            bootstrap_start = len(episode_steps)
            next_state = _clone_state(chunk_steps[-1]["next_state"])

        if bootstrap_start < len(episode_steps):
            next_chunk_steps = episode_steps[bootstrap_start : bootstrap_start + rl_chunk_size]
            next_chunk_actions = _stack_action_chunk(
                [_step_action_tensor(step["executed_action"]) for step in next_chunk_steps],
                rl_chunk_size,
            )
            next_chunk_intervention = any(bool(step["is_intervention"]) for step in next_chunk_steps)
            next_reference_action = (
                next_chunk_actions.clone()
                if next_chunk_intervention
                else _clone_tensor(episode_steps[bootstrap_start]["reference_action"])
            )
        else:
            next_reference_action = _clone_tensor(episode_steps[-1]["reference_action"])

        reference_action = (
            chunk_actions.clone() if chunk_intervention else _clone_tensor(episode_steps[start]["reference_action"])
        )

        transitions.append(
            Transition(
                state=_clone_state(episode_steps[start]["state"]),
                ...
                complementary_info={
                    "discrete_penalty": torch.tensor([chunk_penalty], dtype=torch.float32),
                    "intervention_mask": torch.tensor([[1.0 if chunk_intervention else 0.0]], dtype=torch.float32),
                    TeleopEvents.IS_INTERVENTION: int(chunk_intervention),
    ...
    for interaction_step in range(cfg.policy.online_steps):
        ...
        complementary_info = {
            ...
            "intervention_mask": torch.tensor(
                [float(intervention_info.get(TeleopEvents.IS_INTERVENTION, False))], dtype=torch.float32
            ),
            TeleopEvents.IS_INTERVENTION: int(intervention_info.get(TeleopEvents.IS_INTERVENTION, False)),
        }
        if isinstance(policy, RLTTD3Policy):
            episode_step_records.append(
                {
                    "state": _clone_state(observation),
                    "next_state": _clone_state(next_observation),
                    "executed_action": _clone_tensor(executed_action),
                    "reward": float(reward),
                    "done": bool(done),
                    ...
                    "reference_action": policy.compute_reference_action_chunk(observation)[
                        :, : policy.config.rl_chunk_size
                    ].detach(),
                }
            )
        else:
        ...
        if done or truncated:
            ...
            if isinstance(policy, RLTTD3Policy):
                list_transition_to_send_to_learner = _build_rlt_online_chunk_transitions(
                    policy=policy,
                    episode_steps=episode_step_records,
                )
...
def update_policy_parameters(policy: nn.Module, parameters_queue: Queue, device):
    ...
    if bytes_state_dict is not None:
        ...
        if hasattr(policy, "rl_token_bottleneck") and hasattr(policy, "td3_actor"):
            rl_token_state_dict = move_state_dict_to_device(state_dicts["rl_token_bottleneck"], device=device)
            td3_actor_state_dict = move_state_dict_to_device(state_dicts["td3_actor"], device=device)
            policy.rl_token_bottleneck.load_state_dict(rl_token_state_dict)
            policy.td3_actor.load_state_dict(td3_actor_state_dict)
            logging.info("[ACTOR] Loaded rlt_td3 parameters from Learner.")
            return
