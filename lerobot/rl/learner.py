...
from lerobot.policies.rlt_td3.configuration_rlt_td3 import RLTTD3Config
from lerobot.policies.rlt_td3.modeling_rlt_td3 import RLTTD3Policy
...
from .rlt_learner import (
    make_rlt_td3_loss_components,
    make_rlt_td3_optimizers,
    train_rlt_td3_step,
)


def _rlt_make_policy_batch(sample: dict, input_feature_keys: set[str], include_action: bool) -> dict:
    batch = {key: value for key, value in sample.items() if key in input_feature_keys or key == "task"}
    if include_action:
        batch[ACTION] = sample[ACTION]
    return batch


def _rlt_get_intervention_mask(dataset: LeRobotDataset, index: int) -> bool:
    dataset._ensure_hf_dataset_loaded()
    if "complementary_info.is_intervention" not in dataset.hf_dataset.features:
        return False

    relative_index = index
    if getattr(dataset, "_absolute_to_relative_idx", None) is not None:
        relative_index = dataset._absolute_to_relative_idx[index]

    value = dataset.hf_dataset["complementary_info.is_intervention"][relative_index]
    if torch.is_tensor(value):
        return bool(value.float().max().item() > 0.5)
    if isinstance(value, (list, tuple)):
        return any(float(v) > 0.5 for v in value)
    return bool(float(value) > 0.5)


def _rlt_same_episode_action_chunk(
    ...
    *,
    ...
    chunk_actions = []
    for sample_index in range(start_index, min(start_index + rl_chunk_size, episode_end)):
        sample = dataset[sample_index]
        chunk_actions.append(sample[ACTION].to(dtype=torch.float32))

    if not chunk_actions:
        raise RuntimeError(f"Failed to build RLT action chunk at dataset index {start_index}")

    while len(chunk_actions) < rl_chunk_size:
        chunk_actions.append(chunk_actions[-1].clone())

    return torch.stack(chunk_actions, dim=0).unsqueeze(0)


def _move_batch_tensors_to_device(batch: dict[str, torch.Tensor], device: torch.device) -> dict[str, torch.Tensor]:
    moved = {}
...
def _initialize_rlt_offline_replay_buffer(
    *,
    ...
    preprocessor, _ = make_pre_post_processors(
        ...
        pretrained_path=str(policy.config.pi05_pretrained_path),
    )
    model_device = next(policy.parameters()).device
    input_feature_keys = set(policy.config.input_features.keys())
    rl_chunk_size = policy.config.rl_chunk_size
    discount = policy.config.discount

    total_transitions = sum(
        int(episode["dataset_to_index"]) - int(episode["dataset_from_index"])
        for episode in offline_dataset.meta.episodes
    )
    capacity = max(int(cfg.policy.offline_buffer_capacity), total_transitions)
    ...
    for episode in tqdm(offline_dataset.meta.episodes, desc="Building RLT offline replay"):
        episode_start = int(episode["dataset_from_index"])
        episode_end = int(episode["dataset_to_index"])
        episode_success = str(episode.get("episode_success", "failure")).lower()
        terminal_index = episode_end - 1

        for index in range(episode_start, episode_end):
            current_sample = offline_dataset[index]
            terminal_offset = terminal_index - index
            chunk_reaches_terminal = index + rl_chunk_size > terminal_index
            bootstrap_index = terminal_index if chunk_reaches_terminal else index + rl_chunk_size
            reward = 0.0
            if chunk_reaches_terminal and episode_success == "success":
                reward = discount ** terminal_offset
            done = chunk_reaches_terminal
            next_sample = offline_dataset[bootstrap_index]

            current_policy_batch = preprocessor(
                _rlt_make_policy_batch(current_sample, input_feature_keys, include_action=True)
            )
            next_policy_batch = preprocessor(
                _rlt_make_policy_batch(next_sample, input_feature_keys, include_action=False)
            )
            current_state = {key: value for key, value in current_policy_batch.items() if key != ACTION}

            current_state = _move_batch_tensors_to_device(current_state, model_device)
            next_state = _move_batch_tensors_to_device(next_policy_batch, model_device)

            action = _rlt_same_episode_action_chunk(
                offline_dataset,
                start_index=index,
                episode_end=episode_end,
                rl_chunk_size=rl_chunk_size,
            ).to(model_device)
            next_action = action if done else _rlt_same_episode_action_chunk(
                offline_dataset,
            ...
            current_is_intervention = _rlt_get_intervention_mask(offline_dataset, index)
            next_is_intervention = False if done else _rlt_get_intervention_mask(offline_dataset, bootstrap_index)

            with torch.no_grad():
                reference_action = policy.compute_reference_action_chunk(current_state)[:, :rl_chunk_size].detach()
                next_reference_action = policy.compute_reference_action_chunk(next_state)[:, :rl_chunk_size].detach()

            if current_is_intervention:
                reference_action = action.detach().clone()
            if next_is_intervention:
                next_reference_action = next_action.detach().clone()

            replay_buffer.add(
                state={key: value.detach().cpu() for key, value in current_state.items()},
                action=action.detach().cpu(),
                reward=reward,
                next_state={key: value.detach().cpu() for key, value in next_state.items()},
                done=done,
                truncated=done,
                complementary_info={
                    "reference_action": reference_action.detach().cpu(),
                    "next_reference_action": next_reference_action.detach().cpu(),
                    "intervention_mask": torch.tensor(
                        [[1.0 if current_is_intervention else 0.0]], dtype=torch.float32
                    ),
                },
            )
...
def add_actor_information_and_train(
    ...
    rlt_td3_loss = None
    rlt_td3_target_updater = None
    is_rlt_td3 = isinstance(policy, RLTTD3Policy)
    if is_rlt_td3:
        rlt_td3_loss, rlt_td3_target_updater = make_rlt_td3_loss_components(policy=policy)
        optimizers, lr_scheduler = make_rlt_td3_optimizers(policy, rlt_td3_loss), None
    else:
        ...
        if is_rlt_td3:
            for _ in range(utd_ratio - 1):
                ...
                train_rlt_td3_step(
                    policy=policy,
                    batch=batch,
                    loss_module=rlt_td3_loss,
                    target_updater=rlt_td3_target_updater,
                    optimizers=optimizers,
                    clip_grad_norm=clip_grad_norm_value,
                    update_actor=False,
                )
            ...
            training_infos = train_rlt_td3_step(
                policy=policy,
                batch=batch,
                loss_module=rlt_td3_loss,
                target_updater=rlt_td3_target_updater,
                optimizers=optimizers,
                clip_grad_norm=clip_grad_norm_value,
                update_actor=optimization_step % policy_update_freq == 0,
            )
        else:
            for _ in range(utd_ratio - 1):
...
def initialize_replay_buffer(
    ...
    """
    optimize_memory = not isinstance(cfg.policy, RLTTD3Config)
    ...
    if isinstance(cfg.policy, RLTTD3Config):
        raise NotImplementedError(
            "RLT-TD3 resume from serialized replay datasets is not supported because the generic replay "
            "dataset format does not preserve chunk next_state/reference semantics."
        )
...
def initialize_offline_replay_buffer(
    ...
    optimize_memory = not isinstance(cfg.policy, RLTTD3Config)
    if isinstance(cfg.policy, RLTTD3Config):
        assert isinstance(policy, RLTTD3Policy)
        return _initialize_rlt_offline_replay_buffer(
            cfg=cfg,
            policy=policy,
...
def push_actor_policy_to_queue(parameters_queue: Queue, policy: nn.Module):
    ...
    if hasattr(policy, "rl_token_bottleneck") and hasattr(policy, "td3_actor"):
        state_dicts = {
            "rl_token_bottleneck": move_state_dict_to_device(
                policy.rl_token_bottleneck.state_dict(), device="cpu"
            ),
            "td3_actor": move_state_dict_to_device(policy.td3_actor.state_dict(), device="cpu"),
        }
